...
void couples_binaires(int unique[],int max,int tableau[], int counter, int e_suiv,int wtf[max]) {
    for (int k = 0;k<max;k++){
        int i=0;
                    if (k+1 == e_suiv || k+1 == tableau[counter]){
                        wtf[k]=1;
                        while (unique[i] != k+1){ 
                        //printf(" %d ",unique[i]);    
                            i++;
...
int main() {
    int tableau[] = {1, 2, 7, 2}; // Déclaration et initialisation du tableau - connexe
...
// Déclaration et initialisation d'un tableau dynamique pour stocker les éléments uniques
    ...
    for (int i=0; i< taille_elements_uniques;i++){
        if (tableau_elements_uniques[i] >= max){
  ...
  //parcourir le tableau initiale avec les pairs et les impairs et comparer le courant avec le suivant
    ...
    couples_binaires(tableau_elements_uniques,max,tableau, 0,tableau[1],wt_1);
  ...
  for (int z=2; z < n;z=z+2){
    couples_binaires(tableau_elements_uniques,max,tableau, z,tableau[z+1],wt_1);
